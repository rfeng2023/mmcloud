...
while [[ "$#" -gt 0 ]]; do
    case $1 in
        ...
        --mount-packages) params[mount_packages]="true" ;;
        --float-executable) params[float_executable]="$2"; shift ;;
        -h|--help) usage; exit 0 ;;
...
"${params[float_executable]}" login -a "${params[OP_IP]}" -u "$user" -p "$password"

# Build the float submit command as an array
float_submit_args=(
    "${params[float_executable]}" "submit" "-a" "${params[OP_IP]}"
...
if [[ "${params[mount_packages]}" == "true" ]]; then
    float_submit_args+=(
...
get_public_ip() {
    ...
    while [[ -z "$IP_ADDRESS" ]]; do
        IP_ADDRESS=$("${params[float_executable]}" show -j "$jobid" | grep -A 1 portMappings | tail -n 1 | awk '{print $4}' || true)
...
get_tmate_session() {
    ...
    while true; do
        url=$("${params[float_executable]}" log -j "$jobid" cat stdout.autosave | grep "web session:" | head -n 1 || true)  
        if [[ -n "$url" ]]; then
            ...
            local ssh=$("${params[float_executable]}" log -j "$jobid" cat stdout.autosave | grep "ssh session:" | head -n 1 || true)
...
get_jupyter_token() {
    ...
    while true; do
        url=$("${params[float_executable]}" log -j "$jobid" cat stderr.autosave | grep token= | head -n 1 || true)
        no_jupyter=$("${params[float_executable]}" log -j "$jobid" cat stdout.autosave | grep "JupyterLab is not available." | head -n 1 || true)
...
suspend_command="${params[float_executable]} suspend -j $jobid"
